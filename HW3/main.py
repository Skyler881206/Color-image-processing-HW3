...
if __name__ == "__main__":
    ...
    for index in range(0, len(images), BATCH):
        model = utils.model_arch()  # Read model
        label_list = []
        image_list = []
        ...
        for row in range(3):
            for column in range(5):
                ...
                ax[row, column].set_title("{label}({idx})".format(label=prediction_list[random_list[idx]] + label_list[0],  # Set title for image number and index
                                                                  idx=index + random_list[idx] + 1), color=color)
